...
export default {
  ...
  computed: {
    moveWidth: cur => cur.domainWidth - cur.closeWidth,
    hasFoot: cur => Boolean(cur.buttons.length)
  },
  methods: {
    maskOperator: function(create = true) {
      const parent = this.$el.offsetParent;
      if (!create) {
        this.mask && (this.mask = parent.removeChild(this.mask) && null);
        return;
      }
    ...
    getCloseWidth: function() {
      return this.closable
        ? ["borderLeftWidth", "width", "borderRightWidth"].reduce(
            (res, prop) =>
    ...
    mouseMoveHook: function(ev) {
      const { left, top } = this.getPosition(this.$el.offsetParent);
    ...
    slide: async function(show = true) {
      ...
      const temp = show
        ? {
            fir: wrap,
            sec: domain,
            firOpt: { width: this.domainWidth },
            secOpt: { height: this.domainHeight }
          }
        : {
            fir: domain,
            sec: wrap,
    ...
    push: async function(show = true) {
      ...
      const temp = show
        ? {
            left: pos.left - this.domainWidth,
            top: pos.top
          }
        : {
            left: pos.left,
  ...
  mounted: async function() {
    ...
    this.domainWidth = ["paddingLeft", "width", "paddingRight"].reduce(
      (res, prop) => res + parseFloat(content[prop]),
      0
    );
    this.domainHeight = ["paddingTop", "height", "paddingBottom"].reduce(
      (res, prop) => res + parseFloat(content[prop]),
      0
    );
  ...
  watch: {
    ...
    closable: function() {
      this.$nextTick(() => (this.closeWidth = this.getCloseWidth()));
    },
