...
export default {
    ...
    methods: {
        ...
        onWheel: function(ev) {
            ...
            domain.scrollTop = value <= max && value >= 0 ?
                value : old;
        }
    },
    data: () => ({
    ...
    computed: {
        colorStyle: cur => cur.enter || cur.enable ? 
            cur.enableColor : cur.disableColor,

        titleTop: cur => cur.enable ? 
            5 - cur.height : 0,
        
        textWidth: cur => 
            cur.width - cur.iconWidth,
        ...
        domainHeight: cur => {
            ...
            return cur.height * (len > cur.maximum ?
                cur.maximum : len
            );
        }
    },
    mounted: function() {
        ...
        this.iconWidth = [
            'borderLeftWidth', 'width', 'borderRightWidth'
        ].reduce((res, prop) => 
            res + parseFloat(getComputedStyle(this.$refs.icon)[prop]), 0
        );
    ...
    watch: {
        selectable: function(val) {
            ...
            this.$nextTick(() => {
                if(!this.domainColor) {
                    this.domainColor = getComputedStyle(this.$refs.domain).backgroundColor;
                }
